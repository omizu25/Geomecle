...
void InitFade(void)
{
	...
	// 頂点カラーの設定
	pVtx[0].col = D3DXCOLOR(0.0f, 0.0f, 0.0f, s_alpha);
	pVtx[1].col = D3DXCOLOR(0.0f, 0.0f, 0.0f, s_alpha);
	pVtx[2].col = D3DXCOLOR(0.0f, 0.0f, 0.0f, s_alpha);
	pVtx[3].col = D3DXCOLOR(0.0f, 0.0f, 0.0f, s_alpha);
...
void UpdateFade(void)
{
	...
	case FADE_OUT:	// フェードアウト状態
		s_alpha += ALPHA_CHANGE;	// ポリゴンを不透明にしていく
	...
	case FADE_IN:	// フェードイン状態
		s_alpha -= ALPHA_CHANGE;	// ポリゴンを透明にしていく
